...
# Tentukan posisi 8 buku secara acak - Prioritaskan meja/kasur
all_special = furniture_positions + corner_positions
random.shuffle(all_special)

books = []
# Ambil 8 buku dari pool spesial (furnitur + pojokan) jika cukup
book_pool = all_special[:8] if len(all_special) >= 8 else all_special + random.sample(valid_positions, 8 - len(all_special))
for p in book_pool:
...
book_text = Text(text='Books: 0/8', position=(-0.85, 0.45), scale=2, color=color.white)
...
letter_ui_bg = Entity(parent=camera.ui, model='quad', color=color.white, scale=(0.8, 1.2), z=-10, enabled=False)
letter_ui_text = Text(parent=letter_ui_bg, text='لقد قتلوه\n\n(Mereka telah membunuhnya)\n\nGuru kami dibunuh di sini.', color=color.black, origin=(0,0), scale=2, position=(0,0), z=-0.1)
...
# Foto Besar di Kamar Tidur (Menempel di dinding timur)
creepy_photo = Entity(
    model='quad',
    texture='photo2.png',
    scale=(8, 6), # Ukuran raksasa agar sangat menonjol
    position=(77.9, 3.5, 64), # Di dinding paling belakang (timur), tengah-tengah kasur
    rotation_y=90, # Menghadap ke barat (ke arah pemain)
    double_sided=True, # Pastikan tetap terlihat dari sisi mana saja
    color=color.white
)

# ==== SISTEM EDUKASI NAHWU SHOROF (MATAN JURUMIYAH) ====
qa_data = [
    {
        "question": "Menurut Matan Jurumiyah, bagaimanakah pengertian Kalam?",
        "options": ["Lafadz yang tersusun, berfaedah, dan berbahasa Arab", "Suara yang tidak memiliki makna secara utuh", "Tulisan yang dapat dibaca dan dipahami"],
        "answer": 0
    },
    {
        "question": "Kalam (kalimat) dalam bahasa Arab terbagi menjadi berapa bagian utama?",
        "options": ["Tiga (Isim, Fi'il, Huruf)", "Dua (Isim dan Fi'il)", "Empat (Isim, Fi'il, Huruf, Dhamir)"],
        "answer": 0
    },
    {
        "question": "Di antara berikut ini, manakah yang merupakan tanda-tanda sebuah Isim?",
        "options": ["Khafdh (kasrah), Tanwin, dan kemasukan Alif Lam", "Diawali dengan Qad, Sin, atau Saufa", "Diakhiri dengan Ta' Ta'nits Sakinah"],
        "answer": 0
    },
    {
        "question": "Huruf apakah yang HANYA bisa masuk ke dalam sebuah Isim?",
        "options": ["Huruf Jar (Min, Ila, 'An, 'Ala, dll)", "Huruf Nashab (An, Lan, Idzan, Kay)", "Huruf Jazm (Lam, Lamma, Alam, Alamma)"],
        "answer": 0
    },
    {
        "question": "Manakah di bawah ini yang merupakan tanda khas dari sebuah Fi'il (kata kerja)?",
        "options": ["Bisa menerima Tanwin di akhir kata", "Dapat kemasukan huruf Qad, Sin, dan Saufa", "Selalu diawali dengan huruf Alif Lam (Al-)"],
        "answer": 1
    },
    {
        "question": "Pertanyaan tentang Huruf: Apakah pengertian Huruf menurut ilmu Nahwu?",
        "options": ["Kata yang maknanya baru jelas jika bersambung dengan kata lain", "Kata yang menunjuk pada nama sesuatu atau seseorang", "Kata yang menunjukkan pekerjaan beserta waktunya"],
        "answer": 0
    },
    {
        "question": "Dalam definisi Kalam ada syarat 'Al-Mufidu'. Apa maksudnya?",
        "options": ["Susunan kata yang memberikan kepahaman sempurna", "Lafadz yang tersusun minimal dari tiga kata sempurna", "Sesuatu yang diucapkan dengan bahasa selain bahasa Arab"],
        "answer": 0
    },
    {
        "question": "Khafdh (kasrah) adalah salah satu tanda Isim. Huruf manakah yang BUKAN merupakan Huruf Jar/Khafdh?",
        "options": ["Min, Ila, 'An, 'Ala", "Fi, Rubba, Ba, Kaf, Lam", "Qad, Sin, Saufa"],
        "answer": 2
    }
]

quiz_panel = Entity(parent=camera.ui, model='quad', color=color.rgba32(15, 15, 15, 245), scale=(1.4, 0.9), z=-10, enabled=False)
quiz_title = Text(parent=quiz_panel, text='— P E R T A N Y A A N   M A U T —', position=(0, 0.35), scale=0.9, origin=(0, 0), color=color.red, z=-0.1)
quiz_question_text = Text(parent=quiz_panel, text='', position=(0, 0.15), scale=0.8, origin=(0, 0), color=color.white, z=-0.1)
quiz_buttons = []

from ursina import Button, Func, mouse
for i in range(3):
    btn = Button(parent=quiz_panel, text=' ', position=(0, -0.02 - i*0.16), scale=(0.9, 0.12), color=color.rgba32(70, 45, 45, 230), highlight_color=color.rgba32(150, 40, 40, 240), text_color=color.white, z=-0.1)
...
def show_quiz(book_entity):
    global current_book, quiz_active, current_correct_idx
    current_book = book_entity
    quiz_active = True
    player.disable()
    mouse.locked = False
    
    q = random.choice(qa_data)
    current_correct_idx = q["answer"]
    quiz_question_text.text = q["question"]
    quiz_question_text.wordwrap = 50
    
    prefixes = ['1.', '2.', '3.']
    for i, btn in enumerate(quiz_buttons):
        btn.text = f"{prefixes[i]} {q['options'][i]}"
        if btn.text_entity:
            btn.text_entity.wordwrap = 55
            # Gunakan *= agar aspect ratio (inverse scale) bawaan tombol Ursina tidak hilang!
            btn.text_entity.scale *= 0.7
            btn.text_entity.color = color.white
        btn.on_click = Func(check_answer, i, q["answer"])
...
def check_answer(selected_idx, correct_idx):
    global books_collected, quiz_active, message_timer
    
    if selected_idx == correct_idx:
        pickup_sound.play()
        books_collected += 1
        book_text.text = f'Books: {books_collected}/8'
        
        if books_collected >= 8:
            exit_door.enabled = True
            ...
            message_text.text = 'JAWABAN BENAR!'
            message_text.color = color.green
            message_text.enabled = True
            message_timer = 2.0
            
        books.remove(current_book)
        destroy(current_book)
        close_quiz()
    else:
        ...
        message_text.text = 'SALAH! Darah Berkurang!'
        message_text.color = color.red
        message_text.enabled = True
        message_timer = 2.0
...
# ================= SISTEM DOKUMEN LORE (CERITA) =================
lore_data = [
    "12 Safar 1998\n\nAku merasa terawasi. Padahal aku hanya sedang menghafal pengertian Kalam di perpustakaan pesantren ini. Tapi setiap kali aku menyebut 'Al-Lafdzul Murakkab', ada suara yang menimpali dari balik rak buku...",
    "Katakan Padaku...\n\nUstadz bilang Kitab Jurumiyah adalah kunci kebenaran. Tapi kunci untuk apa? Mengapa setiap kali aku salah menyebutkan Tanwin, bayangan wanita berbaju putih itu mendekat selangkah demi selangkah?",
    "PERINGATAN TERAKHIR\n\nJangan menatapnya! Dia membenci tatapan manusia. Dia terjebak dalam teks Jurumiyah yang telah aku nodai dengan darah. Satu-satunya cara menenangkan roh nahu shorof ini adalah dengan mengembalikan kitab-kitab yang kusembunyikan..."
]

lore_panel = Entity(parent=camera.ui, model='quad', scale=(0.6, 0.8), color=color.rgb(230, 220, 200), enabled=False, z=-20)
lore_text_entity = Text(parent=lore_panel, text=' ', color=color.black, scale=1.5, position=(-0.45, 0.4))
lore_text_entity.wordwrap = 35
lore_instruction_text = Text(parent=lore_panel, text='[Klik Kiri] untuk Menutup', color=color.dark_gray, position=(0, -0.45), origin=(0, 0), scale=1)

def show_lore(content):
    player.disable()
    mouse.locked = False
    lore_text_entity.text = content
...
class LoreNote(Entity):
    def __init__(self, position, content):
        ...
        # Tambahkan cahaya kecil lilin di atas kertas agar terlihat di kegelapan
        self.light = PointLight(parent=self, y=0.5, color=color.rgb(255, 100, 0), radius=3)

lore_notes_list = []
# Sebarkan lore secara acak sepanjang valid_positions
for i, l_text in enumerate(lore_data):
    if valid_positions:
        p = random.choice(valid_positions)
        valid_positions.remove(p)
        lore_notes_list.append(LoreNote(position=p, content=l_text))
...
def update():
    ...
    if quiz_active:
        return # Pause segala elemen permainan (hantu, ketahanan, dll) selama kuis berlangsung!
    ...
    # Deteksi koleksi buku (Kuis Nahwu Shorof)
    for b in list(books):
    ...
    if near_door:
        ...
        if near_key:
            interaction_text.text = '[E] Ambil Kunci'
            interaction_text.enabled = True
            if held_keys['e']:
                keys_collected += 1
                keys_list.remove(near_key)
                destroy(near_key)
                pickup_sound.play()
                key_text.text = f'Keys: {keys_collected}'
                message_text.text = 'Found a Key!'
                message_text.enabled = True
                message_timer = 1.5
                interaction_text.enabled = False
        else:
            # Cek Interaksi Ambil Ammo
            near_ammo = None
            ...
            if near_ammo:
                interaction_text.text = '[E] Ambil Peluru'
                interaction_text.enabled = True
                if held_keys['e']:
                    bullets += 2
                    ammo_boxes_list.remove(near_ammo)
                    destroy(near_ammo)
                    pickup_sound.play()
                    ammo_text.text = f'Ammo: {bullets}'
                    message_text.text = 'Found Ammo!'
                    message_text.enabled = True
                    message_timer = 1.5
                    interaction_text.enabled = False
            else:
    ...
    for ln in list(lore_notes_list):

        if distance(player, ln) < 2.5:
            interaction_text.text = '[F] Baca Catatan'
            interaction_text.enabled = True
            if held_keys['f'] and not lore_panel.enabled:
                show_lore(ln.content)
            break
...
def input(key):
    ...
    if key == 'right mouse down' or key == 'q' or key == 'f':
